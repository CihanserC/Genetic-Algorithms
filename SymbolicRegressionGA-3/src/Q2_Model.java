...
public class Q2_Model extends GPModel {
    // Variables.
    ...
    public Q2_Model(int nofInputBits, String inputsFile, String outputsFile) throws FileNotFoundException {
        // Define function set.
        final List<Node> syntax = new ArrayList<>();
        syntax.add(new AndFunction());
        syntax.add(new OrFunction());
        syntax.add(new XorFunction());
        syntax.add(new NotFunction());
        syntax.add(new ConcatFunction());
        ...
        while ( in.hasNext()) {
            Boolean[] output = new Boolean[variables.length];
            String input = in.next();
            for (int i = 0; i < variables.length; i++) {
                if ( input.charAt(i)=='1')
    ...
    public double getFitness(CandidateProgram p) {
        ...
        if ( program.getReturnType() != String.class )
        ...
        for (int i = 0; i < inputs.size() ; i++ ) {
            ...
            String result = (String)program.evaluate();
            String outputSt = "";
            ...
            for (int j = 0; j < input.length; j++) {
                if(result.charAt(j) != outputSt.charAt(j))
                    totalError++;
            }
        }
        return totalError;
